...
namespace smath {
    ...
    struct alignas(2) float16_t {
        float16_t(float x) { raw = detail::fp16_ieee_from_fp32_value(x); }
    ...
    struct alignas(2) bfloat16_t {
        bfloat16_t(float x) { raw = detail::round_to_nearest_even(x); }
    ...
    inline bfloat16_t operator +(bfloat16_t x, bfloat16_t y) {
        return static_cast<bfloat16_t>(detail::f32_from_bits(x.raw) + detail::f32_from_bits(y.raw));
    }
    inline bfloat16_t operator *(bfloat16_t x, bfloat16_t y) {
        return static_cast<bfloat16_t>(detail::f32_from_bits(x.raw) * detail::f32_from_bits(y.raw));
    }
    ...
    inline float16_t operator +(float16_t x, float16_t y) {
        return static_cast<float16_t>(detail::fp16_ieee_to_fp32_value(x.raw) + detail::fp16_ieee_to_fp32_value(y.raw));
    }
    inline float16_t operator *(float16_t x, float16_t y) {
        return static_cast<float16_t>(detail::fp16_ieee_to_fp32_value(x.raw) * detail::fp16_ieee_to_fp32_value(y.raw));
    }
    ...
    template<>
    struct UseMixedType<float16_t> :std::true_type {};

    template<>
    struct UseMixedType<bfloat16_t> :std::true_type {};

    
    inline bool operator ==(bfloat16_t x, bfloat16_t y) {
        return static_cast<float>(x) == static_cast<float>(y);
    }

    inline bool operator ==(float16_t x, float16_t y) {
        return static_cast<float>(x) == static_cast<float>(y);
    }
